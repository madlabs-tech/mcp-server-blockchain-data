...
One binary, two modes. Configuration is layered: **built-in registry < `config/config.toml` < `config/secrets.toml` < environment** (`BDM__<PATH>`, `__` between segments). Values set by env show as **locked by env** in the dashboard.
...
./target/release/blockchain-data-mcp serve             # or without `serve` for MCP on stdio
```
...
- State: `<data_dir>/bdm.db` (default `./data`): usage counters, client keys, call log. If the directory isn't writable (Claude Desktop starts the binary with cwd `/`) the server keeps counters in memory and warns; pass absolute `--config-dir` and `BDM__SERVER__DATA_DIR`.
...
```bash
docker build -t blockchain-data-mcp .
docker run -d --name ems -p 127.0.0.1:8787:8787 --env-file .env \
  -v bdm-data:/data -v bdm-config:/config blockchain-data-mcp
...
BDM__SERVER__PUBLIC_BIND=0.0.0.0:8787      # inside the container / behind the proxy
BDM__SERVER__ADMIN_BIND=0.0.0.0:8788       # container: published to 127.0.0.1 on the host only
...
**Per vendor** (`BDM__VENDORS__<ID>__…`):

| Setting | Meaning | Example |
|---|---|---|
| `LIMIT__<window>` | the vendor's real quota (defaults to its free tier) | `BDM__VENDORS__ALCHEMY__LIMIT__MONTHLY_CREDITS=30000000` |
| `CAP__<window>` | your budget below the limit; windows `RPS`, `PER_MINUTE`, `DAILY`, `MONTHLY` (`MONTHLY_CREDITS`, `DAILY_REQUESTS` accepted) | `BDM__VENDORS__ALCHEMY__CAP__MONTHLY_CREDITS=15000000` |
| `RESERVE_PCT` | stop routing at `(100 − reserve)%` of the budget | `BDM__VENDORS__ALCHEMY__RESERVE_PCT=10` |
| `ON_EXHAUSTED` | `skip` or `allow_overage` | `BDM__VENDORS__ALCHEMY__ON_EXHAUSTED=skip` |
| `ENABLED` | on/off | `BDM__VENDORS__MORALIS__ENABLED=false` |

Effective budget per window = `min(cap, limit × (1 − reserve_pct/100))`.

**Per client** (`BDM__CLIENTS__DEFAULT__…`, overridable per key in the dashboard or `PATCH /admin/api/clients/{id}`):

| Setting | Example |
|---|---|
| requests per minute (token bucket) | `BDM__CLIENTS__DEFAULT__REQUESTS_PER_MINUTE=30` |
| daily requests (reset 00:00 UTC) | `BDM__CLIENTS__DEFAULT__DAILY_REQUESTS=1000` |
| monthly vendor credits spent for the client (reset on the 1st) | `BDM__CLIENTS__DEFAULT__MONTHLY_CREDITS=200000` |
| tool profile | `BDM__CLIENTS__DEFAULT__TOOL_PROFILE=payments` |

Over a limit → HTTP 429 `QUOTA_EXCEEDED` with `retry_after_secs` and a `Retry-After` header; other clients are unaffected.

**Routing order:** `BDM__ROUTING__DEFAULTS__EVM_RPC=alchemy,quicknode,public` (see README for precedence).
...
# bare metal
blockchain-data-mcp clients create alice --config-dir config
blockchain-data-mcp clients list --config-dir config        # ids, status, names; never keys
```

The key (`bdm_…`) is printed once and stored as a SHA-256 hash.
...
docker compose -f deploy/docker-compose.yml logs -f blockchain-data-mcp
```
...
```bash
sudo cp target/release/blockchain-data-mcp /usr/local/bin/
sudo mkdir -p /etc/blockchain-data-mcp && sudo cp .env /etc/blockchain-data-mcp/env && sudo chmod 600 /etc/blockchain-data-mcp/env
sudo cp deploy/blockchain-data-mcp.service /etc/systemd/system/
sudo systemctl daemon-reload && sudo systemctl enable blockchain-data-mcp
```

The unit runs as a `DynamicUser` with `ProtectSystem=strict`; the only writable path is `/var/lib/blockchain-data-mcp` (`config/` with `admin_token`, `config.toml`, `secrets.toml`; `data/` with `bdm.db`). Set `BDM__SERVER__PUBLIC_BIND=127.0.0.1:8787` in the env file, put a TLS reverse proxy (Caddy, nginx) in front of it and leave `127.0.0.1:8788` unproxied.
...
```bash
sudo systemd-run --wait --pipe --collect -p DynamicUser=yes -p StateDirectory=blockchain-data-mcp \
  -p EnvironmentFile=/etc/blockchain-data-mcp/env -p Environment=BDM__SERVER__DATA_DIR=/var/lib/blockchain-data-mcp/data \
  /usr/local/bin/blockchain-data-mcp clients create alice --config-dir /var/lib/blockchain-data-mcp/config
sudo systemctl start blockchain-data-mcp
sudo journalctl -u blockchain-data-mcp -f
```
...
sudo cat /var/lib/blockchain-data-mcp/config/admin_token # systemd
```
...
```bash
curl -s -H 'Authorization: Bearer bdm_…' -X POST https://api.example.com/v1/wallet/get_balances \
...
- `bdm.db` (sqlite, WAL mode): usage counters, client keys (hashes + limits), call log. Back it up live with sqlite's online backup so the WAL is included:
  ...
  # systemd
  sudo sqlite3 /var/lib/blockchain-data-mcp/data/bdm.db ".backup /root/bdm-$(date +%F).db"
  ```
...
- [ ] `BDM__SERVER__MODE=hosted`; the server refuses to start without client keys.
